...
import { isExamType, isExamVariant, type ExamType, type ExamVariant } from '@/lib/domain/exam-types';
import { validateModules, computeExamTotals, type ParsedModule } from '@/lib/domain/exam-modules';
import { captureException } from '@/lib/infra/observability';
import { syncExamTotals, revalidateExam } from '@/lib/db/exam-totals';

// ─── Seed data ────────────────────────────────────────────────────────────────

type SeedExam = { id: string; title: string; type: ExamType; variant?: ExamVariant; description: string; tag: string; price: number; features: string[] };
...
const SEED_EXAMS: SeedExam[] = [
  ...
  // General Training grades Reading on its own, stricter band table.
  { id: 'ielts-general-1', title: 'IELTS General Training Mock #1', type: 'ielts', variant: 'general', tag: 'IELTS', price: 15, description: 'İmmiqrasiya və peşəkar qeydiyyat üçün IELTS General Training sınağı — Speaking bölməsi yoxdur.', features: ['General Training formatı', 'Letter writing tapşırığı', 'Gündəlik həyat reading mətnləri', 'Band score hesablaması'] },
...
async function requireAdmin() {
  await requireAdminAction('exams');
}
...
function parseBaseFields(formData: FormData) {
  return {
    ...
    variant:     (formData.get('variant') as string) || 'academic',
...
type ValidBaseFields = Omit<BaseFields, 'type' | 'variant'> & { type: ExamType; variant: ExamVariant };
...
function validateBaseFields(f: BaseFields): { error: string } | { fields: ValidBaseFields } {
  ...
  // Only consulted for IELTS, but validated for every type so a bad value can
  // never reach the CHECK constraint as a raw database error.
  if (!isExamVariant(f.variant))
    return { error: 'Yanlış imtahan variantı.' };
  if (isNaN(f.price) || f.price < 0 || f.price > 10_000)
    return { error: 'Qiymət 0 ilə 10 000 arasında olmalıdır.' };
  return { fields: { ...f, type: f.type, variant: f.variant } };
}
...
export async function createExam(_prev: ActionResult, formData: FormData): Promise<ActionResult> {
  ...
  try {
     ...
     *
     * This guard is now largely historical: questions carry a real foreign key
     * to `exams`, and deleting an exam cascades its bank, so a deleted paper can
     * no longer leave one behind. It stays because it produces an explanatory
     * message for the admin rather than a raw constraint violation, and because
     * rows can still arrive by other routes (a restore, a manual insert).
     */
    const [{ n: stranded }] = await db
      .select({ n: count() })
      .from(questionsTable)
      .where(eq(questionsTable.examId, rawExamId));
    if (stranded > 0) {
      return {
    ...
    // 23505 = unique_violation. This was `11000`, Mongo's duplicate-key code,
    // which no Postgres driver ever raises — so the helpful message became
    // unreachable and a duplicate id surfaced as "Server xətası baş verdi".
...
export async function updateExam(examId: string, _prev: ActionResult, formData: FormData): Promise<ActionResult> {
  ...
  try {
    ...
    const [exam] = await db
      .update(examsTable)
      .set({
        title: fields.title, type: fields.type, variant: fields.variant,
        description: fields.description,
        tag: fields.tag, price: String(fields.price), features: fields.features,
...
export async function deleteExam(examId: string): Promise<ActionResult> {
  try {
    ...
    // The ON DELETE RESTRICT foreign key would refuse this anyway; the count is
    // here to say WHY, in a sentence an admin can act on.
    const [{ n: attempts }] = await db
      .select({ n: count() })
     ...
     * actually wants.
     */
    // Also enforced by ON DELETE RESTRICT on `purchases.exam_id`; the count is
    // here so the admin gets a reason rather than a constraint violation.
    const [{ n: purchases }] = await db
      .select({ n: count() })
      .from(purchasesTable)
      .where(and(eq(purchasesTable.examId, examId), ne(purchasesTable.status, 'REFUNDED')));
    if (purchases > 0) {
      return {
    ...
    /*
     * The bank, any in-flight sessions and any audio claims go with it — and not
     * because this function remembers to delete them. `questions.exam_id`,
     * `exam_sessions.exam_id` and `played_audio.exam_id` are all foreign keys
     * declared ON DELETE CASCADE, so Postgres removes them as part of this one
     * statement. (An explicit session delete used to follow, left over from the
     * migration's phased cutover; it had become a no-op.)
     *
     * `exam_results` and `purchases` are ON DELETE RESTRICT, so the two guards
...
export async function seedExams(_prev: SeedResult): Promise<SeedResult> {
  // Writes the whole seed catalog; heavier than an ordinary console action.
  await requireAdminAction('exams-bulk', 'adminHeavy');
  try {
    let created = 0, skipped = 0;
    for (const exam of SEED_EXAMS) {
      const modules = buildModulesForSeed(exam);
      const { totalQuestions, durationMinutes } = computeExamTotals(modules);
      // The unique primary key decides whether this is a create or a skip, so
      // there is no read-then-write to lose a race on.
      const inserted = await db
        .insert(examsTable)
        .values({
          id: exam.id, title: exam.title, type: exam.type,
          variant: exam.variant ?? 'academic',
          description: exam.description, tag: exam.tag, price: String(exam.price),
          features: exam.features, modules, totalQuestions, durationMinutes, isActive: true,
        })
        .onConflictDoNothing({ target: examsTable.id })
        .returning({ id: examsTable.id });
...
export async function resyncExamTotals(
  ...
  try {
    // Recomputes totals for every exam, one syncExamTotals per row.
    await requireAdminAction('exams-bulk', 'adminHeavy');
    const rows = await db.select({ id: examsTable.id }).from(examsTable);
    for (const e of rows) await syncExamTotals(e.id);
    revalidateExam();
